...
- [ ] Add `verify_jwt = false` under `[functions.api]` in `supabase/config.toml`. Add the function-local Deno import map for `postgres` 3.4.9, enable `sloppy-imports` locally for the existing extensionless project imports, use an explicit JSON import attribute in `src/content/packages.ts`, and use explicit `.ts` imports from the Edge entrypoint where Deno requires them. Do not rewrite unrelated imports.
- [ ] Add focused tests for the default fetch export contract, bearer authorization, token fields being present only in the intended auth responses, allowlisted/rejected origins, preflight headers, JSON body translation, query preservation, and error sanitization. Use memory repositories or fake app dependencies; do not connect to Supabase in unit tests.
- [ ] Run `npm run typecheck:server`, `npm test -- --run server/app.test.ts`, and `deno check` against the Edge entrypoint with its import map. Resolve any Deno import/type issue without changing business rules.
...
- [ ] Add the public `VITE_API_BASE_URL` lookup and join it with `/api/*` paths without producing a double slash. An empty/missing value must preserve relative local/Netlify requests.
- [ ] Maintain a module-level `sessionToken` initialized from `sessionStorage.getItem('hoc_vui_session_token')` when available. Wrap storage access so privacy mode, SSR, and test environments do not throw.
- [ ] On every request, send `Authorization: Bearer <sessionToken>` when present, retain `credentials: 'include'`, and send `X-Parent-Grant` only when a parent wrapper explicitly requests it. Never persist `parentGrantToken`.
- [ ] When a successful response contains a string `accessToken`, replace the stored session token. Clear it on logout, credential-change calls, and the existing expired/forbidden `/api/auth/me` path; a network failure must not silently erase a valid token.
- [ ] Preserve all exported wrapper signatures and result shapes used by the React app. Extend only the auth-result types needed to carry `accessToken`; keep `ClientSession` free of the raw token.
- [ ] Add tests covering URL joining, bearer headers after login, session-storage reload behavior, parent-grant memory-only behavior, token replacement after PIN/password change or parent unlock, and logout/expired-session cleanup. Reset storage and mocked fetch between tests.
- [ ] Run `npm test -- --run src/auth/apiClient.transport.test.ts src/auth/apiClient.profile.test.ts` and `npm run typecheck`.
...
- [ ] Add `firebase.json` with `hosting.public = "dist"`, ignores for `firebase.json`, dotfiles, and `node_modules`, a catch-all SPA rewrite to `/index.html`, a no-cache header for `/sw.js`, and immutable one-year headers for hashed `*.js` and `*.css` assets.
- [ ] Do not add Firebase Functions, Firestore, Firebase Auth, a wildcard API rewrite, or a project id to the hosting config.
- [ ] Add `scripts/validate-firebase-hosting.mjs` that parses `firebase.json`, asserts the public directory and SPA rewrite, checks that Service Worker and hashed-asset headers are present, and exits non-zero with a clear message for an invalid configuration. It must not require Firebase CLI or network access.
- [ ] Add `validate:firebase` to `package.json` pointing to that script without changing existing scripts or dependencies.
- [ ] Run `npm run validate:firebase` and inspect the resulting JSON/config diff.
...
- [ ] Document `VITE_API_BASE_URL`, `HOC_VUI_ALLOWED_ORIGINS`, `HOC_VUI_DATABASE_URL`, `SUPABASE_DB_URL`, `HOC_VUI_COOKIE_SECURE`, and `HOC_VUI_DB_POOL_MAX` with placeholders only. State explicitly that `VITE_*` values are public and all database/auth secrets are function-side.
- [ ] Document the deploy order: authenticate/select the Firebase project, deploy the Supabase `api` function with `verify_jwt=false`, set exact allowed origins and least-privilege database secret through Supabase secrets management, smoke-test synthetic accounts, build with the Firebase API URL, deploy Hosting, and verify HTTPS/PWA/mobile behavior.
- [ ] Document checks for login, bearer session after reload, logout, profile update, parent unlock/grant, dashboard read, one synthetic learning-event write, Service Worker, manifest/icons, and SPA deep links.
- [ ] Document rollback by redeploying the last known-good Edge function and Firebase Hosting release or rebuilding with the previous API base URL. Explicitly state that no reset, data import, or real child-data migration is part of this rollout.
- [ ] Update the README's current-state wording so it no longer implies that Firebase/Edge configuration is absent, while keeping the local Netlify fallback instructions.
- [ ] Run a repository search confirming no password-like value or production connection string was added.
...
**Broad-review fix write set:** `supabase/functions/api/index.ts`, `supabase/functions/api/index.test.ts`, `supabase/functions/api/deno.json`, `src/content/packages.ts`, `scripts/check-edge-runtime.mjs`, `package.json`, `docs/deployment/firebase-supabase-edge.md`, `docs/superpowers/plans/2026-09-15-firebase-supabase-edge.md`, `.superpowers/sdd/2026-09-15-firebase-supabase-edge/task-5-fix-report.md`.

- [ ] Review each task's diff against its write set and inspect all generated/config files.
- [ ] Run fresh gates from the feature worktree: `npm test`, `npm run typecheck`, `npm run typecheck:server`, `npm run validate:firebase`, `npm run validate:fox`, and `npm run build`.
- [ ] Run `deno check --unstable-sloppy-imports --import-map supabase/functions/api/deno.json supabase/functions/api/index.ts` and `npm run check:edge-runtime`. The runtime smoke must start `deno serve` on the real Edge entrypoint with a dynamically selected localhost port, send an allowlisted `OPTIONS` that cannot initialize the database, assert HTTP 204 plus exact credentialed CORS headers, and terminate the exact child process with a non-zero result on startup, HTTP, or cleanup failure. If Supabase CLI authentication is available and deployment is separately authorized, deploy only the `api` function to project ref `tvlpabqkternfvsxqovi`; otherwise report the exact authentication/approval gate without weakening local verification.
- [ ] If Firebase CLI authentication and an approved target project are available, run a preview/deploy of Hosting only. Otherwise leave `.firebaserc` uncommitted and report the exact project-selection gate.
- [ ] Verify the built bundle contains the configured public API base URL but no database URL, password, service-role key, or runtime-role credential.
- [ ] Record the final commit hash, test output summary, deployment URLs/status, and any remaining operator action. Keep Netlify/local 8888 rollback path available.
