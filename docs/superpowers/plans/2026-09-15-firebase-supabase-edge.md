...
- [x] Add `verify_jwt = false` under `[functions.api]` in `supabase/config.toml`. Add the function-local Deno import map for `postgres` 3.4.9, enable `sloppy-imports` locally for the existing extensionless project imports, use an explicit JSON import attribute in `src/content/packages.ts`, and use explicit `.ts` imports from the Edge entrypoint where Deno requires them. Do not rewrite unrelated imports.
